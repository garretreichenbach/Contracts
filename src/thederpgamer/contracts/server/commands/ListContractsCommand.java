...
public class ListContractsCommand implements CommandInterface {
    ...
    public String getDescription() {
        return "\n" +
               "- /%COMMAND% [player] [filter...] : Lists the active contracts for the sender (or a player, if specified) as well as their ids, rewards, and progress. Can be filtered to only show specific types.";
    }
    ...
    public boolean onCommand(PlayerState sender, String[] args) {
        if(args == null || args.length == 0) listContracts(sender, ServerDatabase.getPlayerData(sender));
        else {
            PlayerData target = ServerDatabase.getPlayerData(args[0]);
